...
public class CustomHashMap<T> {
    ...
    private int comparacoes; // Implementar para apresentar o número de comparações necessárias para inserir,
    ...
    public T get(long key) {
        // implementar
        // Retorna o valor para o qual a chave especificada é mapeada ou null se este
        // mapa não contém nenhum mapeamento para a chave.
        ...
        for (Dado<T> item : tabela[indice]) {
            ...
            System.out.println("Seu valor é: " + item.valor);
        }
    ...
    public void replace(long key, T value) {
        // implementar
        // Substitui a entrada da chave especificada para o valor V somente se ela
        // estiver mapeada para algum valor.
        int indice = funcaoHash(key);
        for (Dado<T> item : tabela[indice]) {
            if (item.chave == key) {
                item.valor = value;
